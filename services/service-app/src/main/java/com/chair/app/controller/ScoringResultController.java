...
import com.chair.app.model.dto.scoringResult.ScoringResultAddRequest;
import com.chair.app.model.dto.scoringResult.ScoringResultEditRequest;
import com.chair.app.model.dto.scoringResult.ScoringResultQueryRequest;
import com.chair.app.model.dto.scoringResult.ScoringResultUpdateRequest;
import com.chair.app.model.entity.ScoringResult;
import com.chair.app.model.vo.ScoringResultVO;
import com.chair.app.service.ScoringResultService;
...
public class ScoringResultController {
    ...
    public BaseResponse<Long> addScoringResult(@RequestBody ScoringResultAddRequest scoringResultAddRequest, HttpServletRequest request) {
        ...
        // todo 填充默认值
        LoginUserVO loginUser = (LoginUserVO) authAppFeignClient.getLoginUser(request.getHeader("chair-token")).getData();
    ...
    public BaseResponse<Boolean> deleteScoringResult(@RequestBody DeleteRequest deleteRequest, HttpServletRequest request) {
        ...
        LoginUserVO user = (LoginUserVO) authAppFeignClient.getLoginUser(request.getHeader("chair-token")).getData();
        ...
        // 仅本人或管理员可删除
        if (!oldScoringResult.getUserId().equals(user.getId()) && !UserRoleEnum.ADMIN.getValue().equals(user.getUserRole())) {
    ...
    public BaseResponse<Boolean> editScoringResult(@RequestBody ScoringResultEditRequest scoringResultEditRequest, HttpServletRequest request) {
        ...
        ThrowUtils.throwIf(oldScoringResult == null, ErrorCode.NOT_FOUND_ERROR);
        // 仅本人或管理员可编辑
        if (!oldScoringResult.getUserId().equals(loginUser.getId()) && !UserRoleEnum.ADMIN.getValue().equals(loginUser.getUserRole())) {
            throw new BusinessException(ErrorCode.NO_AUTH_ERROR);
        }
        // 操作数据库
        boolean result = scoringResultService.updateById(scoringResult);
